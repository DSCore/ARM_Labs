...
static uint32_t id = 7; //Default to my ID
...
//			id = 7; //id is now set in STATE_4 and defaults to 7
...
//			while(USART2_get_buffer_length() <= 2){}
		...
		case STATE_4_GOT_INPUT:
			...
			USART2_putstr(USART2_get_buffer_as_string());
			//Get the id and set it
			id = USART2_get_buffer_as_number();
