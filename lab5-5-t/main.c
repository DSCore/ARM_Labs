...
void USART2_callback_fn(uint8_t byte)
{
	...
	if(fsm_get_state() == STATE_4){
		USART2_buffer_input(byte);
		USART2_putchar(byte);
		if(USART2_get_buffer_length() >= 2){
			if( fsm_lock() == FSM_LOCK_ACQUIRED ){
				fsm_set_state(STATE_4_GOT_INPUT);
			}
			fsm_unlock();
		}
	}
...
void USART3_callback_fn(uint8_t byte)
{
	...
	else if(fsm_get_state() == STATE_3){
		Wifly_Receive_Update(byte);
	}
}
...
void main(void)
{
	...
	/* Configure user pushbutton and call pushbutton_callback_fn when button press-released */
 	...
 	/* Configure and initialize the input for the potentiometer */
 	ADC_init();

 	/* Configure and initialize the PWM for the server. */
 	PWM_init();
...
//	//Test i2a
//	USART2_putstr("\n\r\0");
//	USART2_putstr(i2a(124));
//	USART2_putstr("\n\r\0");
//	USART2_putstr(i2a(1014));
//	USART2_putstr("\n\r\0");
//	USART2_putstr(i2a(9994));
//	USART2_putstr("\n\r\0");
//	USART2_putstr(i2a(14564));
//	USART2_putstr("\n\r\0");
