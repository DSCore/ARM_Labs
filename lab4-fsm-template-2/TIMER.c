...
void TIMER_init(void(*TIMER_rx_callback)(void)){
	...
	TIM2->DIER |= (0x1); //Interrupt CC1IE enabled
...
//	TIM2->CCER |= TIM2_CCER_CC1E; //TODO: May not need this!
	...
	TIM2->PSC = 15999;//15999; //Set to 15999 so since SYSCLOCK/(PSC+1)=CK_CNT, the rate at which ARR counts, and SYSCLOCK = 16 MHz
	//Set the ARR so that the period is 20 ms
	TIM2->ARR = 1000;//10000; //Set to 10000 so the period is 10 seconds
...
//	TIM2->PSC = 15999; //Set to 15999 so since SYSCLOCK/(PSC+1)=CK_CNT, the rate at which ARR counts, and SYSCLOCK = 16 MHz
	TIM2->ARR = 5000; //Set to 10000 so the period is 10 seconds
